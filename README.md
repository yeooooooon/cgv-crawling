...
4. 영화명, 순위, 에그지수, 누적 관객 수, 관람등급과 URL을 정제
5. 개봉 및 재개봉 여부와 개봉일을 분리
6. 영화명 기준 중복을 제거하고 `data/cgv_movie_clean.csv`로 저장
7. 정제 데이터를 MySQL `movie_chart` 데이터베이스의 `cgv_movies` 테이블에 저장
8. CSV와 MySQL의 저장 건수를 비교해 결과 검증
