...
[개인 프로젝트 — 크롤링부터 전처리, DB 저장, 테스트까지 전 과정 단독 수행]

- Selenium을 활용한 CGV 무비차트 동적 페이지 크롤링 구현
- 순위, 영화명, 에그지수, 누적 관객 수, 관람등급, 개봉 정보, 포스터 URL 등 데이터 추출 로직 구현
- pandas 기반 데이터 전처리(날짜·형식 정리, 결측값 처리, 중복 제거) 구현
- 정제 데이터를 CSV 및 MySQL `cgv_movies` 테이블로 저장하는 로직 구현 및 저장 건수 검증
- pytest 기반 크롤링·전처리·포스터 저장·DB 적재 단위 테스트 작성
- GitHub Actions를 활용한 CI 워크플로우 구성
