...
[개인 프로젝트 — 크롤링, 전처리, 보관, 적재, 배포까지 전 과정을 단독 구성]

- CGV 무비차트 동적 페이지에서 영화 목록 및 메타데이터 수집 로직 구현
- 영화 순위, 제목, 에그지수, 누적 관객 수, 관람등급, 개봉/재개봉 정보, 포스터 URL 추출 기능 개발
- Selenium 기반 페이지 로딩 대기, CSS 선택자 대응, 텍스트 정규화 로직 구성
- pandas를 활용한 데이터 전처리: 공백 정리, 결측값 처리, 중복 제거, 날짜/URL 형식 정제
- 포스터 이미지 다운로드 및 날짜별 폴더 저장 기능 구현
- 정제된 데이터를 CSV로 저장하고 MySQL `cgv_movies` 테이블에 적재하는 파이프라인 구축
- 저장 전후 행 수 검증 로직으로 데이터 누락 여부 확인
- AWS Lambda + SAM + Step Functions 기반 자동 실행 구조 설계 및 배포 구성
- pytest 기반 단위 테스트 작성으로 크롤링, 전처리, 포스터 저장, DB 적재 검증
- GitHub Actions를 활용한 배포 자동화 워크플로우 구성
